...
// 2단계 (병합형): 스크래핑된 여러 본문을 한 번에 분석하여 다수의 레시피 정보 추출
export const buildBatchRecipeAnalysisPrompt = (ingredientsStr: string, sourcesData: { title: string; url: string; bodyText: string }[]) => `
사용자 보유 식재료: [${ingredientsStr}]
...
태스크:
각 [레시피] 항목별로 다음을 수행하세요:
1. 실제 공식 한국어 이름(title)을 추출.
2. 사용자가 보유한 식재료 외에 추가적으로 꼭 필요한 부족 재료들을 추출.
3. 조리 과정을 3~4단계로 직관적으로 요약.
...
반드시 아래 JSON 배열 포맷으로만 출력하세요. 마크다운이나 불필요한 텍스트는 절대 포함하지 마세요.

[출력 형식]
[
  {
    "originalUrl": "해당 레시피의 원본 URL",
    "title": "요리 이름",
